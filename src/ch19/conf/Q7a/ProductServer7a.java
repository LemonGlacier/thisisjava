...
public class ProductServer7a {
	//필드
	private ServerSocket serverSocket;
	private ExecutorService threadPool;
	private List<Product7a> products;
	...
	public class SocketClient{
		...
		public void receive() {
			threadPool.execute(()->{
				try {
					while(true) {
						String recJson = dis.readUTF();
						
						JSONObject request = new JSONObject(recJson);
		...
		public void create(JSONObject request) throws IOException{
			...
			Product7a product = new Product7a();
		...
		public void update(JSONObject request) throws IOException{
			...
			for(int i =0; i<products.size(); i++) {
				Product7a product = products.get(i);
				if(product.getNo()==no) {
					product.setName(data.getString("name"));
					product.setPrice(data.getInt("price"));
					product.setStock(data.getInt("stock"));
		...
		public void delete(JSONObject request) throws IOException{
			...
			JSONObject data =request.getJSONObject("data");
			int no = data.getInt("no");
			Iterator<Product7a> iterator = products.iterator();
			while(iterator.hasNext()) {
				Product7a product = iterator.next();
				if(product.getNo()==no) {
					iterator.remove();
				}
			}
	...
	public static void main(String[] args) {
		ProductServer7a productServer = new ProductServer7a();
		try {
			productServer.start();
		} catch(IOException e) {
